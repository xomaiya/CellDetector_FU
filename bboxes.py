...
def compute_iou(boxes1, boxes2):
    ...
    :param boxes1: A tensor with shape `(N, 4)` representing bounding boxes
        where each box is of the format `[x, y, width, height]`.
    :param boxes2: A tensor with shape `(M, 4)` representing bounding boxes
        where each box is of the format `[x, y, width, height]`.

    :return: pairwise IOU matrix with shape `(N, M)`, where the value at ith row
